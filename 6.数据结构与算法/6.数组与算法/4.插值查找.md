...
+ 步骤1 比例关系：目标值 `target` 位于区间 `[arr[low], arr[high]]` 内

  + 我们可以用 `target` 与 `arr[low]` 和 `arr[high]` 的**相对位置**来估算 `mid`

  + `target` 与 `arr[low]` 的差是 `target - arr[low]`
  + `arr[high]` 与 `arr[low]` 的差是 `arr[high] - arr[low]`

+ 步骤2 目标值位置的比例：`target` 距离 `arr[low]` 的距离与 `arr[low]` 到 `arr[high]` 的距离之间的比例可以表示为：
  ...
  $$
​
+ 步骤3 估算中间位置：根据这个比例，`target` 在整个区间中的位置应该与 `arr[low]` 和 `arr[high]` 之间的距离成比例。因此，我们可以估算 `mid` 的位置为：
...
+ 步骤1 `low = 0`，`high = 9`，`arr[low] = 10`，`arr[high] = 100`，`target = 70`
