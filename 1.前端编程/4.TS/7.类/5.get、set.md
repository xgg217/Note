...
+ 访问器属性如果有set，那么默认get返回具体的类型

## 可选属性 与 get,set 冲突

+ 可选属性这种写法和访问器属性get,set一起写就有逻辑上的冲突

  ```js
  class Animal {
    private _age?: number; // 可选属性与访问器 写法冲突，不客气

    get age() {
      return this._age; // 不能将类型“number | undefined”分配给类型“number”
    }
...
+ 修改方式1：当然就是去掉 `private _age?: number;` 的可选属性，因为这本来就是和访问器属性冲突

  ```js
   class Animal {
    private _age: number; // 去除 可选属性
