...
loader.load("./士兵.glb", function (gltf) {
  ...
  const bone2 = gltf.scene.getObjectByName('mixamorigRightForeArm'); //关节2
  const bone3 = gltf.scene.getObjectByName('mixamorigRightHand'); //关节3
  // // console.log(bone1, bone2, bone3);


  g1.add(bone1.rotation, 'x', 0, Math.PI / 3).name('手臂');
  g1.add(bone2.rotation, 'x', 0, Math.PI / 3).name('肘关节');
  g1.add(bone3.rotation, 'x', 0, Math.PI / 3).name('腕关节');
})
