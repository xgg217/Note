...
+ 返回值
  ...
  dbIndex.get([v1, v2, v3]).onsuccess = e => {
    const data = e.target.result; // 得到符合条件的数据
  }
  ```
...
+ 参数
  ...
  + query
    ...
    + 要查询的键或者 IDBKeyRange
    + 如果传一个有效的键，则会默认为只包含此键的范围
    + 如果此参数不传值，则默认为一个选择了该对象存储空间全部记录的键范围

  + direction 一个 IDBCursorDirection 来告诉游标要移动的方向 有效的值有
...
+ 参数
  ...
  + query 可以是主键值、IDBKeyRange 对象或 null

    + 如果省略或为 null，则遍历所有记录；否则仅处理符合指定键范围的记录

  + direction 指定遍历方向的字符串

    + 默认为 "next"（从前往后）
    + "prev"：从后往前遍历。
    + "nextunique"：从前往后遍历，但跳过重复的键
    + "prevunique"：从后往前遍历，但跳过重复的键
  ...
  request.onsuccess = (event) => {
    ...
    if (cursor) {
      console.log("主键:", cursor.key); // 仅输出主键
      cursor.continue();
    }
  };
  ```
