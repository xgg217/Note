...
+ 在一些低代码或无代码平台中，用户可能会输入自定义的 JavaScript 代码，平台需要即时执行这些代码
+ 通过 new Function，可以将用户输入的字符串直接解析并执行
...
+ 这种方式在在线 IDE 或某些插件系统中较为常见
+ 沙箱的方式有很多种(比如eval，ast 解析实现等等)，new Function 是其中比较简单的一种方式，这里就不细展开说了
